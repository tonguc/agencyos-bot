...
# Regex: ISO, dd/mm/yyyy, dd.mm.yyyy, "12 Mart 2024", "Mart 2024"
_DATE_PATTERNS = [
    r"\b(\d{4})-(\d{2})-(\d{2})\b",                          # 2024-03-12
    r"\b(\d{1,2})[./](\d{1,2})[./](\d{4})\b",                # 12/03/2024 or 12.03.2024
    r"\b(\d{1,2})\s+(%s)\s+(\d{4})\b" % "|".join(TURKISH_MONTHS), # 12 Mart 2024
    r"\b(%s)\s+(\d{4})\b" % "|".join(TURKISH_MONTHS),         # Mart 2024 (day=1 assumed)
]
...
def _try_sitemap(base_url: str) -> dict | None:
    ...
    # <lastmod> taglarını da ayrıca çek
...
def _try_blog(base_url: str) -> dict | None:
    """Blog / haber sayfalarındaki son 3 içeriğin tarihini bul."""
...
async def detect_website_update(url: str) -> dict:
    ...
    # Önce blog dene (en güvenilir, ayrı sayfa)
    ...
    # Header dene (HEAD request, hızlı)
    result = await asyncio.to_thread(_try_header, url)
    if result:
        return result

    # Ana sayfayı çek, footer analizi yap
