...
INFLATION_THRESHOLDS = {
    ...
    "mean_warn": 70.0,
    "high_score_pct_warn": 0.30,  # 90+ skor oranı
}
...
def summarize_score_distribution(results: List[dict]) -> dict:
    ...
    Rejected leadler dahil edilmeli (status='rejected' ile).
    """
    total = len(results)
    if total == 0:
        return {"count": 0}
...
def find_top_score_drivers(results: List[dict]) -> dict:
    """
    score_breakdown varsa katman ortalamalarını hesapla.
    Yoksa opportunity vs intent dağılımı döner.
    """
    if not results:
        return {}
    ...
    if has_breakdown:
        return {
            k: round(mean(v), 1) if v else 0.0
            for k, v in breakdown_totals.items()
        }

    # Fallback: opportunity vs intent split
...
def suggest_rebalancing(summary: dict, inflation: dict) -> List[str]:
    ...
    if "Conversion katmani" in " ".join(reasons):
        ...
        suggestions.append("has_blog False sinyalini sektor bazli yap — tesisatci icin kaldır.")

    if "HOT orani" in " ".join(reasons):
        hot_pct = round(seg_ratio.get("HOT", 0) * 100)
        if hot_pct > 40:
            suggestions.append("HOT esigini 80 → 85 cek.")
        else:
            suggestions.append("HOT esigini 80 → 82 cek.")
    ...
    if "Ortalama skor" in " ".join(reasons):
        ...
        suggestions.append("Audit / pagespeed bonuslarini gozden gecir — zayif site hem audit hem pagespeed aliyor.")

    if "90 uzerinde" in " ".join(reasons):
        suggestions.append("100 cap yeterli degil, soft cap uygula: 85 uzerini yavas artir (diminishing returns).")
...
def format_score_debug_report(summary: dict, inflation: dict, suggestions: List[str]) -> str:
    ...
    driver_lines = "\n".join(
        f"  {k}: {v}" for k, v in sorted(drivers.items(), key=lambda x: -x[1])
        if v > 0
    ...
    reasons_text = (
        "\n".join(f"  - {r}" for r in inflation.get("reasons", []))
        or "  Yok"
    )
